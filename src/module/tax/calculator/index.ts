...
export function calculateTaxSummary(values: TaxOnboardingValues): TaxSummary {
  ...
  const partnerAllowanceShare = roundToCents(clampNonNegative(allowance.baseAllowancePartner))
  ...
  const federalTaxReductionFromAllowances = roundToCents(
    federalTaxReductionFromAllowancesUser + federalTaxReductionFromAllowancesPartner
  )
  ...
  const federalNetTaxUser = roundToCents(
    clampNonNegative(federalGrossTaxUser.total - federalTaxReductionFromAllowancesUser)
  )
  const federalNetTaxPartner = roundToCents(
    clampNonNegative(federalGrossTaxPartner.total - federalTaxReductionFromAllowancesPartner)
  )
  ...
  const netTaxableIncome = federalGrossTaxUser.taxableIncome + federalGrossTaxPartner.taxableIncome
  ...
  if (
    ...
    const penaltyUnrounded = Math.max(0, penaltyGross - reduction) * apPen.finalMitigationFactor
  ...
  return {
    ...
    withholdingTax: roundToCents(
      clampNonNegative(withholdingTax) + clampNonNegative(partnerWithholdingTax)
    ),
