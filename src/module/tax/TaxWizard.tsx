...
export function TaxWizard() {
  ...
  const ippSummary = useMemo(
    () => (!isCompany && step === TAX_WIZARD_TOTAL_STEPS ? calculateTaxSummary(values) : null),
    [isCompany, step, values]
  )
  const companySummary = useMemo(
    () => (isCompany && step === totalSteps ? calculateCompanyTaxSummary(values) : null),
  ...
  const logicInfo = useMemo(
    () => (isCompany ? COMPANY_WIZARD_STEP_LOGIC[step] : TAX_WIZARD_STEP_LOGIC[step]) ?? null,
    [isCompany, step]
  )
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
        <Card>
          ...
          <CardContent>
            ...
            {isCompany && step === 10 ? <Step10CompanyAdditionalIncome /> : null}
            {isCompany && step === 11 ? <Step11CompanySocialContributions /> : null}
