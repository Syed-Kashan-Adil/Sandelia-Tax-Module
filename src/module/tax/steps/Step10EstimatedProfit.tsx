...
export function Step10EstimatedProfit() {
  const selfEmployedStatus = useTaxOnboardingStore(
    (s) => s.values.selfEmployedStatus,
  );
  const companyDirectorRemuneration = useTaxOnboardingStore(
    (s) => s.values.companyDirectorRemuneration,
  );
  const withholdingTax = useTaxOnboardingStore((s) => s.values.withholdingTax);
  const activityStartDate = useTaxOnboardingStore(
    (s) => s.values.activityStartDate,
  );
  const profitEstimationMode = useTaxOnboardingStore(
    (s) => s.values.profitEstimationMode,
  );
  ...
  const ytdProfessionalIncome = useTaxOnboardingStore(
    (s) => s.values.ytdProfessionalIncome,
  );
  const setValues = useTaxOnboardingStore((s) => s.setValues);

  const months = useMemo(
    () => monthsSinceStart(activityStartDate),
    [activityStartDate],
  );
      ...
      <div className="space-y-6">
        ...
        >
          <Input
            ...
            onChange={(e) =>
              setValues({
                companyDirectorRemuneration: Number(e.target.value || 0),
              })
            }
          />
        </Field>
        <Field label="Withholding tax on director remuneration (annual)">
          <Input
            ...
            onChange={(e) =>
              setValues({ withholdingTax: Number(e.target.value || 0) })
            }
          />
        </Field>
    ...
    <div className="space-y-6">
      <div className="grid gap-3">
        ...
        <ModeOption
          ...
          onSelect={() =>
            setValues({ profitEstimationMode: "automatic-extrapolation" })
          }
        />
        <ModeOption
      ...
      >
        <Input
          ...
          onChange={(e) =>
            setValues({
              estimatedProfessionalExpenses: Number(e.target.value || 0),
            })
          }
        />
      </Field>
