      ...
      {p ? (
        <div>
          ...
          <div className="space-y-2 rounded-lg border border-border bg-card p-4 text-sm">
            ...
            <Row
              label="CSSS"
              value={eur(p.specialSocialSecurityContribution)}
            />
            <Row
              label="Withholding tax credit"
            ...
            <div className="border-t border-border pt-2">
              <Row
                label="Estimated personal final balance"
                value={eur(p.finalBalance)}
                strong
              />
            </div>
          </div>
        ...
        <div>
          ...
          <div className="space-y-3 rounded-lg border border-border bg-card p-4 text-sm">
            <div className="space-y-2">
              <Row
                label="1. Mr gross tax (by brackets)"
                value={eur(perPerson.grossMr)}
              />
              <div className="rounded-md border border-border/70 bg-secondary/30 p-3 text-xs text-muted-foreground">
                {p.federalGrossTaxUser.brackets
                  ...
                  .map(
                    (b) =>
                      `${eur(b.amountTaxed)} @ ${(b.rate * 100).toFixed(0)}% = ${eur(b.tax)}`,
                  )
                  .join(" · ")}
              </div>
              <Row
                label="2. Mr allowance used"
                value={eur(perPerson.allowanceMr)}
              />
              <Row
            ...
            <div className="border-t border-border pt-3 space-y-2">
              <Row
                label="5. Mrs gross tax (by brackets)"
                value={eur(perPerson.grossMrs)}
              />
              <div className="rounded-md border border-border/70 bg-secondary/30 p-3 text-xs text-muted-foreground">
              ...
              <Row
                label="6. Mrs allowance used"
                value={eur(perPerson.allowanceMrs)}
              />
              <Row
            ...
            <div className="border-t border-border pt-3 space-y-2">
              ...
              <Row
                label="After set-off"
                value={eur(p.federalTaxTotal - p.withholdingTax)}
              />
              <Row
              ...
              <div className="border-t border-border pt-2">
                <Row
                  label="13. Estimated final result"
                  value={eur(p.finalBalance)}
                  strong
                />
              </div>
              <div className="text-xs text-muted-foreground">
                Marital income splitting:{" "}
                {p.maritalQuotient.applied ? "applied" : "not applied"}
              </div>
            </div>
          </div>
        </div>
