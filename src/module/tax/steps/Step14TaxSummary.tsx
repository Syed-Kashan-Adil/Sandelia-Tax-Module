...
export function Step14TaxSummary(props: { summary: TaxSummary }) {
  ...
  const partnerSalaryNet =
    s.partnerSalariedIncomeGross > 0
      ? round2(
          s.partnerSalariedIncomeGross - s.partnerEmployeeLumpSumDeduction,
        )
      : 0;

  const federalAfterWithholding = round2(
    s.federalTaxTotal -
      s.userWithholdingTax -
      s.partnerWithholdingTax,
  );
      ...
      <div>
        ...
        {isCouple ? (
          ...
          <div className="space-y-2 rounded-lg border border-border bg-card p-4">
            ...
            />
            <SummaryRow label="Federal tax after allowance" value={eur(s.federalTaxTotal)} />
            <SummaryRow
